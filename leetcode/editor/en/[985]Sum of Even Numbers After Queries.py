...
#  
# 
#  Related Topics Array Simulation 👍 2127 👎 323
...
class Solution:
    def sumEvenAfterQueries(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ...
        for q, idx in queries:
            ...
            if is_even and is_even_o:
                even_sum.append(even_sum[-1] + q)
            elif is_even and not is_even_o:
                even_sum.append(even_sum[-1] + num)
            elif not is_even and is_even_o:
                even_sum.append(even_sum[-1] - num_o)
            elif not is_even and not is_even_o:
