...
#  
# 
#  Related Topics Array String Sorting 👍 2157 👎 4407
...
class Solution:
    def reorderLogFiles(self, logs: List[str]) -> List[str]:
        ...
        digit_logs = []
        for log in logs:
            identifier, content = log.split(" ", 1)  # important
            if content.split()[0].isdigit():
                digit_logs.append(log)
            else:
                letter_logs.append((content, identifier))

        letter_logs.sort()  # == letter_logs.sort(lambda tup: (tup[0], tup[1]))
        letter_logs = [f"{_id} {content}" for content, _id in letter_logs]
