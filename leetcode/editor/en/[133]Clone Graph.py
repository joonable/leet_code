...
#  
# 
#  Related Topics Hash Table Depth-First Search Breadth-First Search Graph 👍 99
# 45 👎 4016
...
class Solution:
    def cloneGraph(self, node: Optional['Node']) -> Optional['Node']:
        ...
        #         return None
        #     # 노드 복사 및 저장
        #     copy = Node(node.val, node.neighbors)
        #     copy_nodes[node.val] = copy

        #     # 이웃 dfs -> 이웃들 복사
        #     for neighbor in copy.neighbors:
        #         if not neighbor.val in copy_nodes:
        #             dfs(neighbor)

        #     # 복사가 완료 됐으므로 shallow copy -> deep copy로 바꿔 줌
        #     copy.neighbors = [copy_nodes[neighbor.val] for neighbor in copy.neighbors]
        # dfs(node)
        # return copy_nodes[head.val]
