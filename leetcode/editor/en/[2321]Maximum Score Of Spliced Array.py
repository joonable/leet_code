...
# choose left = 1 and right = 2, nums1 becomes [1,12,13,4,5] and nums2 becomes [11,2,3
# ,14,15]. 
#  
# 
...
#  
# 
#  Related Topics Array Dynamic Programming 👍 830 👎 16
...
class Solution:
    def maximumsSplicedArray(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        curr_min_diff, curr_max_diff = min_diff, max_diff = float("inf"), -float("inf")
        for i in range(n):
            num1, num2 = nums1[i], nums2[i]
            diff = num1 - num2
            curr_min_diff = min(curr_min_diff + diff, diff)
            curr_max_diff = max(curr_max_diff + diff, diff)
            min_diff = min(curr_min_diff, min_diff)
            max_diff = max(curr_max_diff, max_diff)
        return max(sum_nums1 - min_diff, sum_nums2 + max_diff)
